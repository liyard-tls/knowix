...
```
┌─────────────────────────┐
│ ← 12/50 [назва]  [◀ ▶]  │  header (fixed)
│═════════════════════════│
│  Question:              │  питання — sticky вгорі chat area
│  Explain the diff...    │
│─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─│  separator
│  You                    │
│  My answer here...      │
│─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─│
│  Knowix AI              │
│  **Correct!** Here's    │  markdown рендеринг
│  what you got right...  │
│  ```ts                  │  code blocks
│  const x = ...          │
│  ```                    │
│─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─│
│  You                    │  продовження чату після оцінки
│  Can you explain more?  │
│─────────────────────────│
│ [Evaluate] [Send  ↑]    │  input (fixed above keyboard)
└─────────────────────────┘
```

**Правила чату:**
- Чат ніколи не закривається — юзер може продовжувати після оцінки (запитувати, уточнювати)
- Оцінка тригериться: (а) автоматично якщо AI вирішує що відповідь дана, (б) по кнопці "Evaluate"
- Повідомлення **без карток** — на одному фоні, розділяються сепараторами і підписами (You / Knowix AI)
- Markdown рендеринг у всіх AI повідомленнях: `react-markdown` + `remark-gfm`
- Код у `<pre><code>` з синтаксичним виділенням через CSS
- Жирний текст (`**...**`) і акцентні кольори для важливих частин
- **Збереження**: всі повідомлення зберігаються у Firestore (`chatHistory/{uid}_{courseId}_{questionId}`) і відновлюються при перезавантаженні

**Логіка автоматичної оцінки:**
- Prompt передає AI інструкцію: якщо відповідь містить ключові концепції — повернути evaluation JSON, інакше — продовжити діалог звичайним текстом
- AI сам вирішує коли оцінювати — якщо впевнений; юзер завжди може натиснути "Evaluate" явно
